...
class grism_tools:
    ...
    def fit_gaussian(self,wave_min,wave_max,emission=False, use_velocity = False):
        ...
        # Subtract linear slope using edge windows
        x1 = x[4]  ; y1 = np.median(y[0:9]) 
        x2 = x[-4] ; y2 = np.median(y[-10:-1]) 
        ...
        if use_velocity:
            ...
            label=r'FWHM = %.2f+/-%.2f km/s' % (v_fwhm,v_fwhm_err)
            v = c * (x-wave_c) /wave_c
            ax.plot(v,y,'b.')
            if emission:
                ax.plot(v, gauss_em(x,*popt),'k-', label=label)
            else:
                ax.plot(v, gauss_abs(x,*popt),'k-',label=label)
